...
try{
  ...
  $tab = $_GET['tab'] ?? 'shooter';
  $search = trim($_GET['q'] ?? '');
  $demandeurs_filter = isset($_GET['demandeurs']) ? explode(',', $_GET['demandeurs']) : []; // Now refers to NEW demandeur (ex-typologie)
  $demandes_filter = isset($_GET['demandes']) ? explode(',', $_GET['demandes']) : []; // NEW filter for demande (ex-demandeur)
...
if ($tab === 'shooter') {
    // CORRECTION ICI : On ajoute 'à reshooter' dans la liste des statuts acceptés
    $where[] = "p.statut_photo IN ('à shooter', 'à reshooter')";
    
    if ($user['role'] === 'photographe') $where[] = "p.statut_echantillon = 'Reçu'";
    elseif ($user['role'] === 'retoucheur') $where[] = "p.statut_echantillon = 'À mettre DAM'";
  ...
  } elseif ($tab === 'requester') {
    // Onglet Par Demandeur : on veut tout (à shooter, à reshooter, photo ok)
    // Pas de filtre sur statut_photo
  } else {
    // Onglet Photo OK
    $where[] = "p.statut_photo='photo ok'";
  }
  ...
  if ($sort && in_array($sort, ['code_produit', 'designation', 'demandeur', 'demande', 'statut_echantillon', 'deadline', 'date_photo_ok', 'photographe_identifiant'])) {
      $sql .= " ORDER BY $sort $order";
  } else {
      if ($tab === 'shooter' || $tab === 'requester') {
        $sql .= " ORDER BY CASE WHEN p.deadline IS NULL THEN 1 ELSE 0 END, p.deadline ASC, p.code_produit ASC";
      } else {
        $sql .= " ORDER BY p.date_photo_ok DESC, p.code_produit ASC";
      }
  }
